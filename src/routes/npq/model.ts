export function model(time: number, variables: number[], pars: number[]) {
    ...
    let Light_harvesting_complex_protonated: number = LHC_tot - Light_harvesting_complex;
    let Q: number =
    ...
    let keq_b6f: number = Math.exp(
        (2 * E0_PC * F - 2 * E0_PQ * F + 2 * dG_pH * pH_lumen - 2 * dG_pH * (pH - pH_lumen)) / RT
    );
    let keq_fnr: number = Math.exp((-2 * E0_Fd * F + 2 * E0_NADP * F - dG_pH * pH) / RT);
    let vmax_fnr: number = E0_fnr * kcat_fnr;
    let keq_PCP700: number = Math.exp((E0_P700 * F - E0_PC * F) / RT);
    let keq_ferredoxin_reductase: number = Math.exp((-E0_FA * F + E0_Fd * F) / RT);
    let B1: number =
        (PPFD *
            PSII_cross_section *
            PSII_total *
        ...
        (PPFD *
            ...
            Math.pow(PPFD, 2.0) * Math.pow(PSII_cross_section, 2.0) * k2 * keq_Plastoquinone_reduced +
            Plastoquinone_oxidised * Q * k2 * kH * kPQred * keq_Plastoquinone_reduced +
            2.0 * Plastoquinone_oxidised * Q * kF * kH * kPQred * keq_Plastoquinone_reduced +
            2.0 * Plastoquinone_oxidised * Q * kH * kH0 * kPQred * keq_Plastoquinone_reduced +
            Plastoquinone_oxidised *
            Math.pow(Q, 2.0) *
            Math.pow(kH, 2.0) *
            kPQred *
            ...
            Plastoquinone_oxidised * Math.pow(kF, 2.0) * kPQred * keq_Plastoquinone_reduced +
            Plastoquinone_oxidised * Math.pow(kH0, 2.0) * kPQred * keq_Plastoquinone_reduced +
    ...
    let lhc_protonation: number =
        (PsbS_de_protonated * kf_lhc_protonation * Math.pow(protons_lumen, kh_lhc_protonation)) /
        (Math.pow(protons_lumen, kh_lhc_protonation) +
            Math.pow(4000.0 * Math.pow(10.0, -ksat_lhc_protonation), kh_lhc_protonation));
    let lhc_deprotonation: number = PsbS_protonated * kf_lhc_deprotonation;
    let cyclic_electron_flow: number =
        Math.pow(Ferredoxine_reduced, 2.0) * Plastoquinone_oxidised * kf_cyclic_electron_flow;
    ...
    let fnr: number =
        (vmax_fnr *
            ((NADP * Math.pow(Ferredoxine_reduced / km_fnr_Ferredoxine_reduced, 2.0)) / km_fnr_NADP -
    ...
    let proton_leak: number = kf_proton_leak * (protons_lumen - 4000.0 * Math.pow(10.0, -pH));
    ...
    let lhc_state_transition_12: number =
        ...
        (Math.pow(Plastoquinone_oxidised / (PQ_tot * km_lhc_state_transition_12), n_ST) + 1.0);
    let lhc_state_transition_21: number = Light_harvesting_complex_protonated * kPph1;
    ...
    let dPlastoquinone_oxidiseddt: number = -PSII + PTOX + b6f - cyclic_electron_flow - ndh;
    let dPsbS_de_protonateddt: number = lhc_deprotonation - lhc_protonation;
    let dFerredoxine_oxidiseddt: number = -PSI + 2 * cyclic_electron_flow + 2 * fnr;
    let dViolaxanthindt: number = -violaxanthin_deepoxidase + zeaxanthin_epoxidase;
    let dLight_harvesting_complexdt: number = -lhc_state_transition_12 + lhc_state_transition_21;
    return [
        dATPdt,
